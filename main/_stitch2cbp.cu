...
#pragma omp parallel for collapse(2)
    ...
    ss0 << "stitched_image_" << std::setw(4) << std::setfill('0') << 0 << "_"
        << out_width << "x" << out_height;
    std::ostringstream ss180;
    ss180 << "stitched_image_" << std::setw(4) << std::setfill('0')
          << dims[0] - 1 << "_" << out_width << "x" << out_height;

    std::vector<float> projection_0deg(size);
    std::vector<float> projection_180deg(size);
    std::ifstream      file_0deg(getPath(save_stitched_dir, ss0.str()),
                                 std::ios::binary);
    std::ifstream      file_180deg(getPath(save_stitched_dir, ss180.str()),
                                   std::ios::binary);
    ...
    // float max_shift = 200.f;
    // float step_size = 0.1f;
    float max_shift = out_width * max_shift_factor;
    float cor =
        calculateCenterOfRotation(projection_0deg, projection_180deg, out_width,
                                  out_height, max_shift, step_size);
    std::cout << "Calculated Center of Rotation: " << cor << " pixels("
              << cor * pixel_size_mm << " mm)";
    ...
    //! CBP
    start = clock();
    cbp2((int)out_width, (int)out_height, (int)dims[0], pixel_size_mm, cor,
         save_stitched_dir, "stitched_image_", save_voxel_dir);
    end          = clock();
    elapsed_time = double(end - start) / CLOCKS_PER_SEC;
    std::cout << "convolution back projection time: " << elapsed_time
              << " seconds\n";
